...
function paginator($table, $link, $pagelen, $pageno, $fetchpage, $pagevar, $hicolor, $locolor, $pagination_field="name", $target="", $additional_href_vars="")
{
	if ($pagelen > 0) {
		if (!$pageno || $pageno < 0) {
			$pageno = 0;
		}
		$query = "select $pagination_field from $table order by $pagination_field";
		...
		if ($res) {
			...
			if ($npage > 1) {
				echo "<TABLE><TR>\n";
				echo "<TD><B>$table database page</B></TD>\n";
				for ($i=1; $i<=$npage; $i++) {
					mysql_data_seek($res, ($i-1)*$pagelen);
					$row = mysql_fetch_object($res);
					$c = ($i==$pageno)?$hicolor:$locolor;
					echo "<TD BGCOLOR=$c><FONT SIZE=2><A HREF='$fetchpage?$pagevar=$i";
					...
					echo ">", substr($row->$pagination_field, 0, 15),"</A></FONT></TD>\n";
					if ($i > 10 && ($i-1) % 10 == 0) echo "<TR><TD>";
				}
				echo "</TR></TABLE>\n";
