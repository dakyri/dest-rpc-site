...
{
	global	$db_u, $db_p, $usr_u, $usr_p, $usr_p_e;
...
<?php else: ?>
<TD WIDTH=45 VALIGN=top ALIGN=center>
<A HREF="<?php echo $reloff; ?>edit/index.php" CLASS="img-button"><B>User Logon</B></A>
<?php endif; ?>
<TD WIDTH=45 VALIGN=top ALIGN=center>
...
function errorpage($error_message, $error_style="")
{
	$uri = $_SERVER['REQUEST_URI'];
	$urinm = split("/", $uri);
	if (index_of("admin", $urinm) >= 0 || index_of("edit", $urinm) >= 0 || index_of("school", $urinm) >= 0) {
		$reloff = "../";
	} else {
		$reloff = "";
	}
	standard_page_top("In cyberspace, no-one can hear you scream....",
		$error_style?$error_style:"style/default.css",
		"page-main",
		$reloff."images/title/site_error.gif",
		560, 72,
		"Site Error");
	echo "<br clear=all>";
	echo "<P>Unfortunately, it isn't possible to fulfill this request.";
	if ($error_message) {
		echo "The reason may well be:<BR><B>$error_message</B><BR>";
	}
?>
<BR>
<CENTER>
<a href="#top" width="40" height="40" border="0" title="Return to previous page"  class="img-button" onclick="history.back()">Go Back</A>
</CENTER>
<?php
	exit;
}
